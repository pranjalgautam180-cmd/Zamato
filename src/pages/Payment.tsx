...
export default function Payment() {
  ...
  useEffect(() => {
    if (payment?.provider !== 'UPI' || !payment.paymentUrl) {
      setUpiQrCode(null);
      return;
    }

    QRCode.toDataURL(payment.paymentUrl, {
      width: 240,
      margin: 2,
      errorCorrectionLevel: 'M',
    }).then(setUpiQrCode).catch(() => setCheckoutError('Unable to generate the UPI QR code.'));
  }, [payment]);

  useEffect(() => {
    if (paymentData?.payment?.provider !== 'UPI' && paymentData?.payment && !checkoutOpened && !checkoutError) {
  ...
  const isUpiPayment = payment?.provider === 'UPI';
              ...
              {isUpiPayment ? (
                <div className="space-y-3">
                  <div className="rounded-2xl bg-gray-50 p-4 border border-gray-200">
                    ...
                    <p className="text-sm text-gray-900">UPI</p>
                  </div>
                  {upiQrCode && (
                    <div className="rounded-2xl border border-gray-200 bg-white p-4 text-center">
                      <img src={upiQrCode} alt="UPI payment QR code" className="mx-auto h-52 w-52" />
                      <p className="mt-3 text-sm text-gray-600">Scan with any UPI app to pay ₹{totalAmount}.</p>
                    </div>
                  )}
                  <a
                    href={payment?.paymentUrl}
                    className="block w-full rounded-2xl bg-red-500 px-5 py-3 text-center font-semibold text-white hover:bg-red-600 transition-colors"
                  >
                    Pay with UPI App
                  </a>
                </div>
