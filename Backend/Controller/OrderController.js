...
async function CreateOrder(req, res) {
  try {
    ...
    if (paymentMode === 'ONLINE' && !RAZORPAY_ENABLED) {
      return res.status(503).json({ error: 'Online payments are not configured. Please choose cash on delivery.' });
    }

    if (paymentMode === 'UPI' && !UPI_ID) {
      return res.status(503).json({ error: 'UPI payments are not configured. Please choose cash on delivery.' });
    }
    ...
    if (paymentMode === 'UPI') {
      const paymentUrl = new URLSearchParams({
        ...
        am: total.toFixed(2),
        cu: 'INR',
        tn: `Food order ${created.orderNumber}`,
      });
      ...
      return res.status(201).json({
        ...
        payment: {
          provider: 'UPI',
          paymentUrl: `upi://pay?${paymentUrl.toString()}`,
        },
      });
    }
